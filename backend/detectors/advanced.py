...
ONLY runs AFTER removing legitimate accounts and already-assigned ring accounts.
Only flags communities with density > 0.5 AND high internal flow.
"""
...
def detect_community_suspicion(
    tg,
    min_community_size: int = 3,
    min_density: float = 0.5,
    exclude_nodes: Set[str] = None,
    already_in_ring: Set[str] = None,
) -> List[Dict[str, Any]]:
    """
    Detect suspicious communities using graph clustering.

    ONLY runs AFTER:
      - Removing legitimate accounts
      - Removing accounts already in cycle/fan_in_fan_out/shell rings

    Only creates community ring if:
      - density > 0.5
      - AND total internal flow is high
    """
    results = []
    exclude = exclude_nodes or set()
    ring_accounts = already_in_ring or set()

    # Build subgraph excluding legitimate and already-assigned accounts
    skip = exclude | ring_accounts
    remaining_nodes = [n for n in tg.G.nodes() if n not in skip]

    if len(remaining_nodes) < min_community_size:
        return results

    subgraph_dir = tg.G.subgraph(remaining_nodes)
    undirected = subgraph_dir.to_undirected()
    ...
    for community in communities:
        ...
        # Community-level metrics on the DIRECTED subgraph
        sub_dir = tg.G.subgraph(community_nodes)
        internal_edges = sub_dir.number_of_edges()

        # Density (directed)
        max_edges = len(community_nodes) * (len(community_nodes) - 1)
        density = internal_edges / max(max_edges, 1)

        # *** STRICT: only flag if density > threshold ***
        ...
        # Risk: dense, isolated communities with high internal flow
        risk = min(1.0,
            density * 0.35
            + internal_ratio * 0.30
